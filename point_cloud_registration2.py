...
def pointCloudRegistration(prefix, name, hyperparams):
    ...
    for i in range(2):
        ...
        upPCR(tgt_pcd, pcd_reg)

        k



def upPCR(tgt_pcd, pcd_reg):
    pcd_point = np.asarray(pcd_reg.points)
    tgt_point = np.asarray(tgt_pcd.points)
    json_path = "./data/bunny-pcd/points.json"
    
    data = json.load(open(json_path, 'r'))
    p1_if = data['pcd_1']
    p2_if = data['pcd_2']
    p3_if = data['pcd_3']
    p1_ids = p1_if['all_idxs']
    p2_ids = p2_if['all_idxs']
    p3_ids = p3_if['all_idxs']
    tgt_sapoint = np.array([tgt_point[row] for row in p1_ids])
    pcd_sapoint = np.array([pcd_point[row] for row in p2_ids])

    R, t = icp(pcd_sapoint, tgt_sapoint, pcd_point, tgt_point)
